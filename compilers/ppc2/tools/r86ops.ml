...
let storec ra b =
  if is8bit ra then
    ...
    let ra' = if reg_of b = AX then DX else AX in
    push ra'; move ra' ra; 
    put "movb $, $" [fReg8 ra'; fAddr b]; 
    pop ra'
  end
...
let call =
  function
      Const f -> put "call $" [fStr f]
    | Reg r -> put "call *$" [fReg r]
    | _ -> failwith "call"
...
let optab =
  mktable [copy ".text"; copy ".global"; copy ".data"; copy ".common";
    copy ".align"; copy ".string"; copy ".byte";
    ("prolog", 
    ...
    ("not", OpRR (fun ra rb -> compare "sete" ra rb (Const "0")));
