...
type code =
  ...
  | LDN of int * int		(* LDN (n, s) = CONST n / PLUSA / LOAD s *)
  | STN of int * int		(* STN (n, s) = CONST n / PLUSA / STORE s *)
...
let fInst =
  function
    ...
    | LDN (n, s) ->	fMeta "LDN$ $" [fType s; fNum n]
    | STN (n, s) ->	fMeta "STN$ $" [fType s; fNum n]
