...
(* |has_value| -- check if def_kind is suitable for use in expressions *)
let has_value = 
  function
      ConstDef _ | VarDef | CParamDef | VParamDef | StringDef _ -> true 
    | _ -> false
...
let rec is_var e =
  match e.e_guts with
	...
	begin
	  match d.d_kind with 
      	      VarDef | VParamDef | CParamDef -> true
	    | _ -> false
	end
    | Sub (a, i) -> is_var a
    | Select (r, x) -> is_var r
...
let rec check_expr env e =
  begin
	...
	Variable x -> 
	  ...
	  if not (has_value d.d_kind) then
	    sem_error "$ is not a variable" [fId x.x_name];
	  begin
	    match d.d_kind with 
		ConstDef v -> e.e_value <- Some v 
	      | _ -> ()
	  end;
	  d.d_type
      | Sub (e1, e2) ->
      ...
      | Number n -> e.e_value <- Some n; integer
      | Char c -> e.e_value <- Some (int_of_char c); character
...
and check_arg env (formal, arg) =
  match formal.d_kind with
	...
	if formal.d_kind = VParamDef && not (is_var arg) then
	  sem_error "var parameter is not a variable" [];
    | PParamDef ->
...
and check_libcall env q args v =
  ...
  match q.q_id with 
    ...
    | ReadChar ->
	if not (is_var (List.hd args)) then
	  sem_error "parameter of read_char is not a variable" []
    | NewProc ->
	let t1 = check_expr env (List.hd args) in
	if not (is_pointer t1) || not (is_var (List.hd args)) then
	  sem_error "parameter of new is not a pointer variable" []
    | ArgvProc ->
	let t1 = check_expr env (List.nth args 0)
	and t2 = check_expr env (List.nth args 1) in
	if not (same_type t1 integer) || not (is_string t2)
	    || not (is_var (List.nth args 1)) then
...
let rec check_stmt env s =
  ...
  match s.s_guts with
	...
	if not (is_var lhs) then
	  sem_error "assigning to a non-variable" [];
...
let rec check_typexpr env = 
  function
    ...
    | Record fields ->
	let size = ref 0 in
	let env' = check_decls (upward_alloc size) (new_block env) fields in
	let r = { r_size = !size; r_align = max_align } in
	mk_type (RecordType (top_block env')) r
    | Pointer te ->
	let t = 
	  match te with
	      TypeName x ->
...
and check_heading env (Heading (x, fparams, result)) =
  ...
  incr level;
  let pcount = ref 0 in
  let env' = check_decls (param_alloc pcount) (new_block env) fparams in
  decr level;
  ...
  mk_type (ProcType { p_fparams = top_block env'; 
		      p_pcount = !pcount; p_result = rt }) proc_rep

(* |check_decls| -- check a sequence of declarations *)
and check_decls alloc ds = List.fold_left (check_decl alloc) ds

(* |check_block| -- check a local block *)
let rec check_block rt env (Block (ds, ss, fsize)) =
  let env' = check_decls (downward_alloc fsize) (new_block env) ds in
...
let annotate (Prog (Block (globals, ss, _), glodefs)) =
  level := 0;
  let env = check_decls global_alloc (new_block init_env) globals in
