...
and block = Block of decl list * stmt * int ref
...
and caselab = Single of expr | Range of expr * expr
...
(* |Stmt| -- construct a stmt node *)
let makeStmt (s, n) = { s_guts = s; s_line = n }

(* |dummy| -- dummy definition *)
let dummy =
  { d_tag = new_ident "*dummy*"; d_kind = DummyDef; d_type = voidtype;
    d_level = -1; d_addr = Nowhere }

(* |MakeName| -- contruct a name node with dummy annotations *)
let makeName (x, n) = { x_name = x; x_line = n; x_def = dummy }

(* |MakeBlock| -- construct a block node with summy annotations *)
let makeBlock (decls, stmts) = Block (decls, stmts, ref 0)
...
let rec fBlock (Block (decls, stmts, _)) =
  match decls with
...
let print_tree fp (Prog (body, _)) = 
  fgrindf fp "(PROGRAM $)" [fBlock body]
