...
(*
Environments are represented using CAML Light's library module that
...
let add_block b (Env (b0, m)) = Env (List.rev b, List.fold_left add_def m b)
...
let find_def (x, s) ds =
  let rec search =
	...
	[] -> raise Not_found
      | d::ds -> 
	  let (y, t) = d.d_tag in
	  if x = y then d else search ds in
  search ds
...
let replace d (Env (b, m)) =
  ...
  Env (repl b, add_def m d)

let lookup x (Env (b, m)) = IdMap.find x m

let arid = Env ([], IdMap.empty)
